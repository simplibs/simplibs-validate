Zhodnocení nápadu: souhlasím, dává to smysl, a tady je proč — narozdíl od `rules/` vrstvy (kde konstrukce kódu *je* to zajímavé — jak `AllOf` flattenuje, jak `Compose` řetězí transformaci a validaci) je `tools/` vrstva zajímavá svým **chováním navenek a rozhodnutími designu**, ne mechanikou uvnitř. Kód tam navíc není jedna kompaktní `is_valid` metoda, ale rozvětvené wrappery (sync/async větve, `@overload` páry, pomocné funkce volané napříč více soubory) — vytrhávat z toho úryvky by čtenáři nedalo ucelený obrázek, jen fragmenty. Slovní popis procesu ("kdy se co kompiluje, kdy se co loguje, co se stane při chybě") je tady skutečně srozumitelnější než kód. Jeden společný soubor navíc dává smysl i proto, že tyhle nástroje spolu úzce spolupracují (`override_rules`/`validated_type` produkují vstupy pro `validate_call`/`validate_dataclass`), takže je fajn je mít pohromadě, ne roztroušené v pěti souborech.
