...
# Tested function
from simplibs.validate.tools.validate_call._helpers.is_bypass_parameter import (
    is_bypass_parameter,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_make_param("validate", inspect.Parameter.empty), True),
        (_make_param("validate", bool), True),
        # Escape hatches - named "validate" but with non-bool annotation
        (_make_param("validate", int), False),
        (_make_param("validate", str), False),
        (_make_param("validate", list[str]), False),
        # Ordinary parameters
